...
if (params.aligner == 'bwa-mem') {
    ...
    include { BOWTIE2 } from './modules/alignReadsBowtie2'
    include { BOWTIE_INDEX } from './modules/BowtieIndex'
} else {
...
workflow TRIM_QC {
    ...
    if (params.trim) {
        ...
        fastqc_files = FASTQC(merged_reads_ch).collect()

        MULTI_QC(fastqc_files)
...
workflow {
    ...
    if (params.aligner == 'bowtie2') {
        ...
        if (params.index_genome){
            // Flatten as is of format [fasta, [rest of files..]]
            indexed_genome_ch = BOWTIE_INDEX(params.genome_file).flatten()
        } else {
            indexed_genome_ch = Channel.fromPath(params.bowtie_index_files)
        }    
        }
    ...
    if (params.aligner == 'bwa-mem') {
        ...
        hashtable = DRAGMAP_HASH(indexed_genome_ch.collect())
    } else if (params.aligner == 'bowtie2') {
        align_ch = BOWTIE2(read_pairs_ch, indexed_genome_ch.collect())
    }
